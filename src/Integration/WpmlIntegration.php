...
class WpmlIntegration extends AbstractMigrator {
    ...
    public function migrate(): array {
        ...
        if ( $product_total > 0 ) {
            ...
            if ( ! empty( $product_rows ) ) {
                ...
                // Do NOT query OC DB here — content already in WP postmeta from main migration.
                // OC remote connection (13.206.54.252) takes 40s+ = kills PHP timeout.
                // postmeta (_octowoo_name_ar, _octowoo_description_ar) is the source of truth.
                // If postmeta is empty → English content used as Arabic fallback (correct behavior).
    ...
    private function translatePostsFromRows(
        ...
        foreach ( $rows as $row ) {
            ...
            if ( $sec_content !== '' ) {
                $has_arabic = preg_match( '/[\x{0600}-\x{06FF}]/u', $sec_content ) ? '[ARABIC]' : '[ENGLISH fallback]';
                $this->logger->debug( "[multilingual] content #{$primary_id}: {$has_arabic}" );
            }
